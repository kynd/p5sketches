...
function drawStarMarker(p, size) {
  beginShape();
  for (let i = 0; i < 5; i ++) {
    let a0 = Math.PI * 2 / 5 * i;
    let a1 = Math.PI * 2 / 5 * (i + 0.5);
    vertex(p.x + Math.sin(a0) * size * 1.5, p.y - Math.cos(a0) * size * 1.5);
    vertex(p.x + Math.sin(a1) * size * 0.5, p.y - Math.cos(a1) * size * 0.5);
  }
  endShape(CLOSE);
}
...
class Tween {
  ...
  createCubicBezier(a, b, resolution = 40) {
    ...
    return (t)=>{
      t = Math.max(0, Math.min(t, 1));
      let ti = Math.floor(t * resolution);
