...
let gain = 0.1;
let phase = 0;
let labels = ["A", "A#", "B", "C", "C#", "D", "D#", "E", "F", "F#", "G", "G#", "A(Octave)"];
...
function draw() {
  ...
  let freq = Math.pow(2.0, pitch / 12.0) * baseFreq;
  ...
  drawLabel(8, 32, labels[pitch] + " = " + freq.toPrecision(4) + "Hz", LEFT);
...
function audiohandler(event) {
  ...
  let acc = Math.pow(2.0, pitch / 12.0) * baseFreq / audioCtx.sampleRate;
